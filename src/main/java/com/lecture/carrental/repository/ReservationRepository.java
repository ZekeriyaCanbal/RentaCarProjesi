...
public interface ReservationRepository extends JpaRepository<Reservation, Long> {
    ...
    // @Query("SELECT new com.lecture.carrental.domain.dto.ReservationDTO(r) FROM Reservation r")
    List<ReservationDTO> findAllBy();

    Optional<ReservationDTO> findByIdOrderById(Long id) throws ResourceNotFoundException;

    // @Query("SELECT new com.lecture.carrental.domain.dto.ReservationDTO(r) FROM Reservation r WHERE r.id =?1 and r.userId=?2")
    List<ReservationDTO> findAllByUserId(User userId);

    // @Query("SELECT new com.lecture.carrental.domain.dto.ReservationDTO(r) FROM Reservation r WHERE r.userId.id =?1")
