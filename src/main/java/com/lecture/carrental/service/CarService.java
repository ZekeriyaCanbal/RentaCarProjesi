...
import com.lecture.carrental.domain.dto.CarDTO;
...
public class CarService {
    ...
    public CarDTO findById(Long id) throws ResourceNotFoundException{
        ...
        return carRepository.findByIdOrderById(id).orElseThrow(()->new ResourceNotFoundException(String.format(CAR_NOT_FOUND_MSG, id)));
    }
    ...
    public void updateCar(Long id, String imageId, Car car) throws BadRequestException {
        ...
        Car carInfo = carRepository.getById(id);        //eski araç bilgisine ulaş

        if (carInfo.getBuiltIn()) {
            throw  new BadRequestException("You dont have permission to update car!");
        }

        // car.setBuiltIn(false);  //default olarakta false gelir. İsterseniz eklenmeye bilir.
        Set<FileDB> fileDBS = new HashSet<>();
    ...
    public void removeById(Long id) throws BadRequestException {
        Car car = carRepository.findById(id).orElseThrow(()->new ResourceNotFoundException(String.format(CAR_NOT_FOUND_MSG, id)));
